...
// the payment period length this payment was expected to last
    ...
    void createMap(InvoiceDTO invoice, BigDecimal amount) {
        ...
        if (Integer.valueOf(payment.getPaymentResult().getId()).equals(Constants.RESULT_FAIL) || Integer.valueOf(payment.getPaymentResult().getId()).equals(Constants.RESULT_UNAVAILABLE)) {
            ...
            while (task != null && processorUnavailable) {
                ...
                if (auth != null) {
                    ...
                    if (!processorUnavailable) {
                        if (Integer.valueOf(info.getPaymentResult().getId()).equals(Constants.RESULT_FAIL)) {
                ...
                if (!processorUnavailable && Integer.valueOf(info.getPaymentResult().getId()).equals(Constants.RESULT_FAIL)) {
    ...
    public void delete() throws SessionInternalError {
        ...
        try {
            ...
            payment.setUpdateDatetime(Calendar.getInstance().getTime());
            payment.setDeleted(Integer.valueOf(1));
