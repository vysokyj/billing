...
public class UserBL extends ResultList implements UserSQL {
    ...
    private Integer create(Integer entityId, String userName, String password,
        ...
        for (Integer roleId: roles) {
        	RoleDTO dto = new RoleDAS().findByRoleTypeIdAndCompanyId(roleId, entityId);
        	//Possible fix pending cause analysis of this situation for 4965 - cant create partner.
        	if(dto==null){
        		LOG.error("No role DTO found for roleID: "+roleId);
        		throw new IllegalStateException("No role with ID "+roleId+" found. Please verify database configuration and setup");
        	}
            rolesDTO.add(dto);
        }
        ...
        if (allowedRetries > 0) {
            ...
            if (total >= allowedRetries) {
                ...
                eLogger.auditBySystem(user.getEntity().getId(), user.getId(),
                        Constants.TABLE_BASE_USER, user.getUserId(),
                        EventLogger.MODULE_USER_MAINTENANCE,
                        EventLogger.ACCOUNT_LOCKED, Integer.valueOf(total),
                        null, null);
