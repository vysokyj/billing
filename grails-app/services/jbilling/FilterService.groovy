...
class FilterService implements Serializable {
    ...
    def void setFilter(FilterType type, Filter filter, boolean reset = true) {
        ...
        if (filters) {
        	//Fix for issue: Exception is occurred when click top search icon of the page (4926)
        	//Convert any other collection types (Such as persistent sets) to arraylist so we can do indexOf
        	log.debug("Filters type is : "+filters.getClass())
